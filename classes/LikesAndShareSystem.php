...
class Lass_LikesAndShareSystem
{
	...
	public function __construct() {
		...
		// first check if woo action hooks exists
		add_action('init', array($this, 'lass_woo_actions_check'));
	...
	public function lass_woo_actions_check() {
		if ( Lass_WoocommerceSettings::CheckWooCommerceExist() === true ) {
			add_action( $this->plugin_settings['lass_woo_position'], array($this, 'output_lass_to_posts_woo'), 0);

			if ( $this->plugin_settings['lass_woo_archive'] === 'yes' ) {
				add_action( $this->plugin_settings['lass_woo_archive_position'], array($this, 'output_lass_to_posts_woo'), 100);
			}
		}
	}

	public function output_lass_to_posts_woo() {
		if ( Lass_WoocommerceSettings::CheckWooCommerceExist() === true ) {
			global $post;
	...
	public function lass_archive_social_excluded( $active_social = '' ) {
		...
		if ( Lass_WoocommerceSettings::CheckWooCommerceExist() === true ) {
			...
			if ( is_product_category() || is_shop() || is_product_tag() ||
			     is_category() || is_archive() || is_post_type_archive() || is_tag() || is_tax() ) {
				if ( in_array($active_social, $this->plugin_settings['lass_social_sharing_network_exclude']) ) {
					$output = true;
				}
			} else if ( is_product()  ) {
				if ( array_key_exists('name',  $woocommerce_loop) ) {
					if ( $woocommerce_loop['name'] === 'related' || $woocommerce_loop['name'] === 'products' ) {
						if ( in_array($active_social, $this->plugin_settings['lass_social_sharing_network_exclude']) ) {
							$output = true;
						}
					}
				}
			} else if ( $woocommerce_loop !== null && array_key_exists('name',  $woocommerce_loop) ) {
				if ( $woocommerce_loop['name'] == 'products' ) {
					if ( in_array( $active_social, $this->plugin_settings['lass_social_sharing_network_exclude'] ) ) {
						$output = true;
					}
				}
